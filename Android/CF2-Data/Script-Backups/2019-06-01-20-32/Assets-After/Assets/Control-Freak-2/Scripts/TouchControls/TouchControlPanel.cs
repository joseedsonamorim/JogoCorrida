		...
		private bool IsTouchPressureSensitive(int touchId, out float pressureOut)	
			...
			if (UnityEngine.Input.touchPressureSupported)
				...
				for (int i = 0; i < ControlFreak2.CF2Input.touchCount; ++i)
					{
					ControlFreak2.InputRig.Touch t = ControlFreak2.CF2Input.GetTouch(i);
		...
		public void Update()
			...
			if ((this.elapsedSinceLastAction += Time.unscaledDeltaTime) > TouchControlPanel.staticFingerTimeout) //this.panel.staticFingerTimeout)
				...
				if (!ControlFreak2.CF2Input.GetMouseButton(0) && 
					!ControlFreak2.CF2Input.GetMouseButton(1) &&
					!ControlFreak2.CF2Input.GetMouseButton(2) && 
					(ControlFreak2.CF2Input.touchCount == 0))
					{ 
#if UNITY_EDITOR
