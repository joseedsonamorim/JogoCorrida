...
public class ResonanceAudioDemoManager : MonoBehaviour {
  ...
  void Update() {
    ...
    if (cubeHit) {
      if((ControlFreak2.CF2Input.touchCount == 0 && ControlFreak2.CF2Input.GetMouseButtonDown(0)) ||    // LMB for desktop.
         (ControlFreak2.CF2Input.touchCount > 0 && ControlFreak2.CF2Input.GetTouch(0).tapCount > 1 &&   // Double-tap for mobile.
          ControlFreak2.CF2Input.GetTouch(0).phase == TouchPhase.Began)) {
